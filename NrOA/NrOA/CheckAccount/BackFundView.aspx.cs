...
namespace NrOA.CheckAccount
{
	...
	/// BackFundView 的摘要说明。
	/// </summary>
	public class BackFundView : System.Web.UI.Page
	{
		...
		NrOA.CheckAccount.em.BackFundInfo bfi = new NrOA.CheckAccount.em.BackFundInfo();
		...
		private void Add()
		{
			...
			TextBox4.Text = ((NrOA.CheckAccount.em.ChargeInfo)(list)[0]).PaymentMonth;
		...
		private void save()
		{
			...
			CI.PaymentMonth = TextBox4.Text;//年结费用
			...
			bfi.ClientName = DropDownList5.SelectedItem.Text;//客户名称

			bfi.ClientOperationID = TextBox2.Text;//客户业务号

			bfi.Now_Poundage = DropDownList3.SelectedValue;	//分成比例							

			bfi.BackFund = TextBox4.Text;//回款

			bfi.BackFund_Year =  DropDownList1.SelectedValue;//回款年份

			bfi.BackFund_Month = DropDownList2.SelectedValue;//回款月份

			bfi.RegisterMane = ht["EmployeeName"].ToString();

			bfi.RegisterTime = TextBox12.Text;
			...
			else
			{
				...
				doci.Update_Charge_BackMonthPay(CI);
				dobfi.update_BackFundInfoList(bfi);
			}
